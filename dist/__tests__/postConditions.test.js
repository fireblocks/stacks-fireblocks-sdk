...
describe("FT Post-Conditions", () => {
    ...
    describe("Pc builder for FT transfers", () => {
        it("creates a post-condition with correct principal", () => {
            const postCondition = transactions_1.Pc.principal(senderAddress)
                .willSendEq(amount)
        ...
        it("handles different token contract formats", () => {
            // sBTC format
            const sbtcPostCondition = transactions_1.Pc.principal(senderAddress)
                .willSendEq(amount)
                .ft("SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token", "sbtc-token");
            // USDCx format
            const usdcxPostCondition = transactions_1.Pc.principal(senderAddress)
                .willSendEq(amount)
    ...
    describe("PostConditionMode enum", () => {
        it("has Allow mode with value 1", () => {
            expect(transactions_1.PostConditionMode.Allow).toBe(1);
        });
        it("has Deny mode with value 2", () => {
            expect(transactions_1.PostConditionMode.Deny).toBe(2);
        });
        it("Deny mode is stricter than Allow", () => {
            ...
            expect(transactions_1.PostConditionMode.Deny).toBeGreaterThan(transactions_1.PostConditionMode.Allow);
        });
    });
    describe("Post-condition array structure", () => {
        ...
        it("supports multiple post-conditions", () => {
            const ftPostCondition = transactions_1.Pc.principal(senderAddress)
                .willSendEq(amount)
                .ft(`${contractAddress}.${contractName}`, contractName);
            const stxPostCondition = transactions_1.Pc.principal(senderAddress)
