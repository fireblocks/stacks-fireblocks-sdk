...
describeE2E("E2E: STX Transfer", () => {
    ...
    const waitForTxConfirmation = async (sdk, txId, timeoutMs = TX_CONFIRMATION_TIMEOUT) => {
        var _a, _b, _c;
        ...
        while (Date.now() - startTime < timeoutMs) {
            ...
            if ((_a = status.error) === null || _a === void 0 ? void 0 : _a.includes("404")) {
            ...
            if (txStatus === "abort_by_response" || txStatus === "abort_by_post_condition") {
                throw new Error(`Transaction failed: ${((_c = status.data) === null || _c === void 0 ? void 0 : _c.tx_error) || txStatus}`);
            }
            await new Promise((resolve) => setTimeout(resolve, TX_POLL_INTERVAL));
        }
    ...
    it("should transfer STX from Vault A to Vault B", async () => {
        ...
        const result = await sdkA.createNativeTransaction(addressB, TRANSFER_AMOUNT, false, // not gross transaction
        "E2E Test: A to B");
    ...
    it("should transfer STX from Vault B back to Vault A", async () => {
        ...
        const result = await sdkB.createNativeTransaction(addressA, TRANSFER_AMOUNT, false, // not gross transaction
        "E2E Test: B to A");
        expect(result.success).toBe(true);
        expect(result.txHash).toBeDefined();
        console.log(`Transaction submitted: ${result.txHash}`);
