...
exports.getPoolMetrics = exports.replaceTransaction = exports.getPoxInfo = exports.extendStackingPeriod = exports.increaseStackedAmount = exports.stackSolo = exports.getTxStatusById = exports.revokeDelegation = exports.allowContractCaller = exports.delegateToPool = exports.createTransaction = exports.getTransactionHistory = exports.getFtBalances = exports.getBalance = exports.getAccountNonce = exports.getPublicKey = exports.checkStatus = exports.getBtcRewardsAddress = exports.getAddress = void 0;
...
// GET /:vaultId/nonce
const getAccountNonce = async (req, res, next) => {
    try {
        const vaultId = getVaultId(req);
        const result = await apiService.executeAction(vaultId, types_1.ActionType.GET_ACCOUNT_NONCE, {});
        res.json(Object.assign(Object.assign(Object.assign({}, result), (result.confirmedNonce !== undefined && { confirmedNonce: result.confirmedNonce.toString() })), (result.nextAvailable !== undefined && { nextAvailable: result.nextAvailable.toString() })));
    }
    catch (err) {
        next(err);
    }
...
const createTransaction = async (req, res, next) => {
    try {
        ...
        const tokenAssetName = req.body.tokenAssetName
            ...
            : undefined;
        const nonce = (0, validation_1.parseOptionalNonce)(req.body.nonce);
        const fee = (0, validation_1.parseOptionalFee)(req.body.fee);
        ...
        if (tokenType === types_2.TokenType.STX) {
            const tx = await apiService.executeAction(vaultId, types_1.ActionType.CREATE_NATIVE_TRANSACTION, { recipientAddress, amount, grossTransaction, note, nonce, fee });
            res.json(tx);
            return;
        }
        // FT transfer
...
const delegateToPool = async (req, res, next) => {
    try {
        ...
        const tx = await apiService.executeAction(vaultId, types_1.ActionType.DELEGATE_TO_POOL, { poolAddress, poolContractName, amount, lockPeriod, nonce });
        res.json(tx);
    }
    catch (err) {
        next(err);
...
const allowContractCaller = async (req, res, next) => {
    try {
        ...
        const poolAddress = constants_1.poolInfo[poolType].poolAddress;
        const poolContractName = constants_1.poolInfo[poolType].poolContractName;
        const tx = await apiService.executeAction(vaultId, types_1.ActionType.ALLOW_CONTRACT_CALLER, { poolAddress, poolContractName, nonce });
        res.json(tx);
    }
    catch (err) {
        next(err);
...
const revokeDelegation = async (req, res, next) => {
    ...
    try {
        const vaultId = getVaultId(req);
        const nonce = (0, validation_1.parseOptionalNonce)((_a = req.body) === null || _a === void 0 ? void 0 : _a.nonce);
        const tx = await apiService.executeAction(vaultId, types_1.ActionType.REVOKE_DELEGATION, { nonce });
        res.json(tx);
    }
    catch (err) {
        next(err);
...
const replaceTransaction = async (req, res, next) => {
    try {
        const vaultId = getVaultId(req);
        ...
        const newFee = (0, validation_1.parseOptionalFee)(req.body.newFee);
        const newAmount = (0, validation_1.parseOptionalFee)(req.body.newAmount);
        const nonceOverride = (0, validation_1.parseOptionalNonce)(req.body.nonceOverride);
        ...
        const tx = await apiService.executeAction(vaultId, types_1.ActionType.REPLACE_TRANSACTION, { originalTxId, newFee, newRecipient, newAmount, nonceOverride });
        res.json(tx);
    }
    catch (err) {
        next(err);
