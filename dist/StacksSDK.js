...
class StacksSDK {
    constructor(vaultAccountId, fireblocksConfig, hiroApiKey) {
         ...
         */
        this.buildSignSendTransfer = async (recipientAddress, microAmount, type = types_1.TransactionType.STX, token, customTokenContractAddress, customTokenContractName, customTokenAssetName, note) => {
            try {
                const transactionToSign = await this.chainService.serializeTransaction(this.address, this.publicKey, recipientAddress, microAmount, type, token, customTokenContractAddress, customTokenContractName, customTokenAssetName);
         ...
         * @returns - A promise that resolves to the transaction broadcast result.
         */
        this.buildSignSendContractCall = async (functionName, poolAddress, poolContractName, amount, maxAmount, lockPeriod, extendCycles, signerKey, signerSig65Hex, startBurnHeight, authId, contractCallParams, note) => {
            try {
                if (functionName === "allow-contract-caller" &&
                ...
                switch (functionName) {
                    case "allow-contract-caller":
                        transactionToSign = await this.chainService.allowPoxContractCaller(this.publicKey, poolAddress, poolContractName);
                        break;
                    case "delegate-stx":
                        transactionToSign = await this.chainService.delegateStx(this.publicKey, poolAddress, amount, lockPeriod);
                        break;
                    case "revoke-delegate-stx":
                        transactionToSign = await this.chainService.revokeStxDelegation(this.publicKey);
                        break;
                    case "solo-stack":
                        transactionToSign = await this.chainService.soloStack(this.publicKey, signerKey, amount, this.btcRewardsAddress, lockPeriod, maxAmount, // maxAmount ( >= amount )
                        signerSig65Hex, startBurnHeight, authId);
                        break;
                    case "increase-stack-amount":
                        transactionToSign = await this.chainService.increaseStackedStx(this.publicKey, signerKey, amount, maxAmount, signerSig65Hex, authId);
                        break;
                    case "extend-stack-period":
                        transactionToSign = await this.chainService.extendStackingPeriod(this.publicKey, signerKey, this.btcRewardsAddress, extendCycles, maxAmount, signerSig65Hex, authId);
                        break;
                ...
                const rawSignature = await this.fireblocksService.signTransaction(transactionToSign.preSignSigHash, this.vaultAccountId.toString(), note || "");
                const signature = (0, helpers_1.concatSignature)(rawSignature.fullSig, rawSignature.v);
                transactionToSign.unsignedContractCall.auth.spendingCondition.signature = (0, transactions_1.createMessageSignature)(signature);
         ...
         */
        this.createNativeTransaction = async (recipientAddress, amount, grossTransaction = false, note) => {
         ...
         * @throws {Error} If the address, public key, or vault ID are not set, or if the transaction creation fails.
         */
        this.createFTTransaction = async (recipientAddress, amount, token, customTokenContractAddress, customTokenContractName, customTokenAssetName, note) => {
            if (!this.address || !this.publicKey || !this.vaultAccountId) {
                throw new Error("Address, Public Key or Vault ID are not set");
            }
            ...
            try {
                ...
                const microAmount = paramsValidationResponse.finalAmount;
                const result = await this.buildSignSendTransfer(recipientAddress, microAmount, types_1.TransactionType.FungibleToken, token, customTokenContractAddress, customTokenContractName, customTokenAssetName, note);
                if (!result || result.error || !result.txid || result.reason) {
         ...
         */
        this.delegateToPool = async (poolsAddress, poolContractName, amount, lockPeriod) => {
            var _b;
            if (this.testnet) {
            ...
            try {
                ...
                const delegateResult = await this.buildSignSendContractCall("delegate-stx", poolsAddress, poolContractName, (0, helpers_1.stxToMicro)(amount), undefined, lockPeriod);
         ...
         */
        this.allowContractCaller = async (poolsAddress, poolContractName) => {
            if (this.testnet) {
            ...
            try {
                ...
                const allowCallerResult = await this.buildSignSendContractCall("allow-contract-caller", poolsAddress, poolContractName);
        ...
        this.revokeDelegation = async () => {
            ...
            try {
                ...
                const revokeResult = await this.buildSignSendContractCall("revoke-delegate-stx");
         ...
         */
        this.stackSolo = async (signerKey, signerSig65Hex, amount, maxAmount, lockPeriod, // Number of cycles
        authId) => {
            var _b, _c;
            try {
                if (!this.address || !this.publicKey || !this.vaultAccountId) {
                ...
                const result = await this.buildSignSendContractCall("solo-stack", undefined, undefined, (0, helpers_1.stxToMicro)(amount), (0, helpers_1.stxToMicro)(maxAmount), lockPeriod, undefined, signerKey, signerSig65Hex, startBurnHeight, authId);
         ...
         * @returns A response indicating success or failure of the transaction.
         */
        this.increaseStackedAmount = async (signerKey, signerSig65Hex, increaseBy, maxAmount, authId) => {
            var _b, _c;
            try {
                if (!this.address || !this.publicKey || !this.vaultAccountId) {
                    throw new Error("Address, Public Key or Vault ID are not set");
                ...
                const result = await this.buildSignSendContractCall("increase-stack-amount", undefined, undefined, (0, helpers_1.stxToMicro)(increaseBy), (0, helpers_1.stxToMicro)(maxAmount), undefined, undefined, signerKey, signerSig65Hex, undefined, authId);
                const assertResult = (0, helpers_1.assertResultSuccess)(result);
                if (assertResult.success === false) {
                    return {
                        success: false,
        ...
        * @returns A response indicating success or failure of the transaction.
        */
        this.extendStackingPeriod = async (signerKey, signerSig65Hex, extendCycles, maxAmount, authId) => {
            var _b, _c;
            try {
                if (!this.address || !this.publicKey || !this.vaultAccountId) {
                    throw new Error("Address, Public Key or Vault ID are not set");
                ...
                const result = await this.buildSignSendContractCall("extend-stack-period", undefined, undefined, undefined, (0, helpers_1.stxToMicro)(maxAmount), undefined, extendCycles, signerKey, signerSig65Hex, undefined, authId);
                const assertResult = (0, helpers_1.assertResultSuccess)(result);
                if (assertResult.success === false) {
                    return {
                        success: false,
        ...
        */
        ...
        this.makeContractCall = async (contractAddress, contractName, functionName, functionArgs, postConditions, postConditionMode) => {
            try {
                ...
                const result = await this.buildSignSendContractCall("generic-contract-call", undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, { contractAddress, contractName, functionName, functionArgs, postConditions, postConditionMode });
                const assertResult = (0, helpers_1.assertResultSuccess)(result);
                if (assertResult.success === false) {
                    return {
                        success: false,
